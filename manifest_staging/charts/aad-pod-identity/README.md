...
| `rbac.createUserFacingClusterRoles` | If set to true, then view and edit cluster roles will be created with annotations that agrigate to the admin, edit and view built-in Kubernetes cluster roles. These roles will be able to create the necessary resources to allow pod identity binding on pods. | `true` |
