...
echo "Checking to ensure the Azure CLI is installed and in PATH"
if ! command az > /dev/null 2>&1; then
    echo "'az' was not found in PATH. Please install the Azure cli before running this script."
    exit 1
fi
echo "Found az in PATH"
echo "\n"
echo "Checking to ensure that kubectl cli is installed and in PATH"
...
do
  echo "Sleeping for 30 seconds waiting for AAD Propogation of Identity"
  sleep 30s
done
echo "\n"
...
EOF
echo "\n"
echo "Deploying aadpodidentitybinding resource into the default namespace"
if kubectl get deploy mic > /dev/null 2>&1; then
...
echo "To verify Identity try \`kubectl get AzureIdentity\` and \`kubectl get AzureIdentityBinding\`"
